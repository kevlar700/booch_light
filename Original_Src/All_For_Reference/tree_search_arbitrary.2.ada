...
package body Tree_Search_Arbitrary is
    ...
    begin
        if not Is_Null (The_Tree) then
            ...
            for Index in 1 .. Number_Of_Children_In (The_Tree) loop
                Traverse_Pre_Order (Child_Of (The_Tree, The_Child => Index));
            end loop;
        end if;
    ...
    begin
        if not Is_Null (The_Tree) then
            for Index in 1 .. Number_Of_Children_In (The_Tree) loop
                Traverse_Post_Order (Child_Of (The_Tree, The_Child => Index));
            end loop;
            Process (The_Tree, Continue);
            if not Continue then
                return;
