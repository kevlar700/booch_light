...
package body String_Multiple_Unbounded_Unmanaged_Iterator is
    ...
    procedure Iterate (Over_The_String : in String) is
        Continue : Boolean;
    begin
        Seize_For_Reading (Over_The_String);
        for The_Iterator in 1 .. Over_The_String.The_Length loop
            Process (Over_The_String.The_Items (The_Iterator), Continue);
            exit when not Continue;
        end loop;
        Release_For_Reading (Over_The_String);
    end Iterate;

end String_Multiple_Unbounded_Unmanaged_Iterator;
