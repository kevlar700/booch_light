...
package Pipe_Unbounded_Unmanaged is
    ...
    function Extent_Of (The_Pipe : in Pipe) return Natural;
...
private
    task type Pipe is
        entry Read       (The_Item  : out Item);
        entry Write      (The_Item  : in  Item);
        entry Get_Extent (The_Value : out Natural);
    end Pipe;
